...
UMMF::UML::Export - Base exporter class for UMMF::UML::MetaModel;

=head1 SYNOPSIS

  use base qw(UMMF::UML::Export::...);
  my $code = UMMF::UML::Export::...->new(...);
  $code->export_Model($model);

=head1 DESCRIPTION

This package allow UML (meta-)models to be exported, to XMI or implementation languages.
...
use base qw(UMMF::UML::MetaMetaModel::Configurable);
...
sub initialize
{
  ...
  $self->{'output'} ||= *STDOUT;
  $self->{'output'} = UMMF::UML::Export::EvalIO->new()
...
sub model_filter
{
  ...
  if ( @filters ) {
    ...
    for my $filter ( @filters ) {
      unless ( ref($filter) ) {
	$filter = "UMMF::UML::XForm::$filter" unless $filter =~ /::/;
...
package UMMF::UML::Export::EvalIO;
